...
window.copyOCRResult = (index, imgId) => {
    ...
    navigator.clipboard.writeText(textArea.value).then(() => {
        showToast(t("toast.ocrCopied"), "fa-check");
    });
};
...
window.performOCR = async (id) => {
    ...
    try {
        ...
        const text = data.candidates?.[0]?.content?.parts?.[0]?.text || t("ocr.noText");
        ...
        console.error(err);
        alert(t("alert.ocrFailed", { message: err.message }));
    } finally {
        btn.innerHTML = `<i class="fa-solid fa-font"></i> ${t("ocr.fullPageButton")}`;
        btn.disabled = false;
    }
};
...
window.performCropOCR = async (imgId, objIndex, event) => {
    ...
    if (!state.apiConfig.key || state.apiConfig.key.includes('{') || state.apiConfig.key.includes('fetch')) {
        alert(t("alert.apiKeyMissing"));
        toggleSettingsModal();
        return;
    }
    
    showToast(t("toast.ocrRunning"), "fa-circle-notch fa-spin");
    ...
    try {
        ...
        showToast(t("toast.ocrCompleted"), "fa-check");
